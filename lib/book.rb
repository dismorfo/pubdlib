...
class Book
  ...
  def initialize(se, ticket, script = 'Latn')
    ...
    se.fmds.each do |fmd|
      if fmd.name === "#{se.identifier}_hi.pdf"
        pdfs.push(
          type: 'hi',
          uri: "pdfserver://books/#{se.identifier}/#{se.identifier}_hi.pdf",
      ...
      if fmd.name === "#{se.identifier}_lo.pdf"
        pdfs.push(
          type: 'lo',
          uri: "pdfserver://books/#{se.identifier}/#{se.identifier}_lo.pdf",
          filesize: fmd.filesize,
          searchable: fmd.searchable
        )
      end
    ...
    item_data = {
      ...
      metadata: {
        ...
        publication_date_text: {
          label: 'Date of Publication (string)',
          value: mets.publication_date_string
        },
        publication_date: {
          label: 'Date of Publication',
          value: mets.publication_date(mets.publication_date_string)
        },
        call_number: {
          label: 'Call Number',
      ...
      series: mets.series(se.collection_code, se.partner_code)
    }
    # multivolume: {
    #   volume: mods_doc.get_multivolume(
    #     @id,
    #     book[2],
    #     book[1],
    #     collection_code,
  ...
  def save_to_file
    File.write(
      "#{$configuration['CONTENT_DIR']}/books/#{@data.identifier}.#{@data.language}.json",
      JSON.pretty_generate(@data)
    )
    puts "Saved book entity to file: #{$configuration['CONTENT_DIR']}/books/#{@data.identifier}.#{@data.language}.json"
  end
end
